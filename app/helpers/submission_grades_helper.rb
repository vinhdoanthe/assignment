module SubmissionGradesHelper
  ...
  def student_can_submit?(assignment_id, user_id)
    ...
    if last_submission_grade.nil? or (last_submission_grade.submission_status == Constants::SUBMISSION_GRADE_STATUS_NOTPASSED and last_submission_grade.attempt_count < Assignment.find(assignment_id).max_attempt)
      true
    else
      false
    end
