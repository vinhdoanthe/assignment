module SubmissionGradesHelper
  ...
  def student_can_submit?(assignment_id, user_id)
    ...
    if (last_submission_grade.nil? || (last_submission_grade.status == Constants::SUBMISSION_GRADE_STATUS_NOTPASSED && (Assignment.find(assignment_id).max_attempt.zero? ? true : last_submission_grade.attempt_count < Assignment.find(assignment_id).max_attempt)))
      true
    else
      false
    end
